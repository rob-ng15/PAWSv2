...
algorithm audio(
    ...
    // SHARED FREQUENCY TABLE - FIRST 12 NOTES, SHIFT RIGHT TO GET ADDITIONAL OCTAVES
    uint12 frequency_table[] = {
        0,
        2986, 2819, 2660, 2511, 2314, 2237, 2112, 1993, 1881, 1776, 1676, 1582     // 1 = C 2 or Deep C ( rest generated by shifts )
    };
    octave CPUINPUT( frequency <: frequency );                  uint12  CPUfrequency <:: frequency_table[ CPUINPUT.basefrequency ] >> CPUINPUT.shift;
    octave SAMPLELEFT( frequency <: samples_left.rdata0 );      uint12  LEFTfrequency <:: frequency_table[ SAMPLELEFT.basefrequency ] >> SAMPLELEFT.shift;
    octave SAMPLERIGHT( frequency <: samples_right.rdata0 );    uint12  RIGHTfrequency <:: frequency_table[ SAMPLERIGHT.basefrequency ] >> SAMPLERIGHT.shift;

    // SINE WAVE GENERATOR - EFFECTIVELY IMPLEMENTED AS A DUAL PORT BROM
    sinegenerator SINE( point0 <: apu_processor_L.point[1,7], point1 <: apu_processor_R.point[1,7] );

    // Left and Right audio channels
    // 8 bit output from audio channel goes through PWM modulator
    apu apu_processor_L( staticGenerator <: staticGenerator, sine <: SINE.sine0, audio_active :> audio_active_l );
    apu apu_processor_R( staticGenerator <: staticGenerator, sine <: SINE.sine1, audio_active :> audio_active_r );
    audio_pwm PWM_L( wave <: apu_processor_L.audio_output, audio :> audio_l );
    audio_pwm PWM_R( wave <: apu_processor_R.audio_output, audio :> audio_r );
    ...
    always_after {
        ...
        if( SAMPLEMODE_L ) {
            if( ~apu_processor_L.audio_active ) {
                ...
                apu_processor_L.frequency = LEFTfrequency;
                apu_processor_L.apu_write = 1;
                if( SAMP0P1 == samples_MAX_L ) { SAMPLEMODE_L = 0; } else { samples_left.addr0 = SAMP0P1; }
            }
        } else {
        ...
        if( SAMPLEMODE_R ) {
            if( ~apu_processor_R.audio_active ) {
                ...
                apu_processor_R.frequency = RIGHTfrequency;
                apu_processor_R.apu_write = 1;
                if( SAMP1P1 == samples_MAX_R ) { SAMPLEMODE_R = 0; } else { samples_right.addr0 = SAMP1P1; }
            }
        } else {
