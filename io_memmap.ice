...
algorithm audio(
    ...
    // SHARED FREQUENCY TABLE - FIRST 12 NOTES, USE OCTAVE ALGORITHM TO GET ADDITIONAL NOTES
    ...
    octave SAMPLELEFT( frequency <: samples_left.rdata0 );      octave SAMPLERIGHT( frequency <: samples_right.rdata0 );

    // SINE WAVE GENERATOR - EFFECTIVELY IMPLEMENTED AS A DUAL PORT BROM
    sinegenerator SINE( point0 <: apu_processor_L.point[2,6], point1 <: apu_processor_R.point[2,6] );
    ...
    always_after {
        ...
        if( SAMPLEMODE_L ) {
            if( ~apu_processor_L.audio_active ) {
                ...
                apu_processor_L.frequency = frequency_table[ SAMPLELEFT.basefrequency ] >> SAMPLELEFT.shift;
        ...
        if( SAMPLEMODE_R ) {
            if( ~apu_processor_R.audio_active ) {
                ...
                apu_processor_R.frequency = frequency_table[ SAMPLERIGHT.basefrequency ] >> SAMPLERIGHT.shift;
