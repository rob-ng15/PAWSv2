...
int main( int argc, char **argv ) {
    ...
    while( !( get_buttons() & 4 ) ) {
        ...
        update(); move_sprites();
        if( !(systemclock()&15) ) {
            tpu_set( 14, 1, TRANSPARENT, WHITE ); tpu_outputstring( 1, "Based upon https://www.lexaloffle.com/bbs/?tid=35767" );
            tpu_set( 28, 2, TRANSPARENT, WHITE ); tpu_outputstring( 1, "Written by @tommulgrew" );
        } else {
