...
int xoffset, yoffset;
int vwidth = 320, vheight = 200; /* size of screen */
int viewsize;
...
configend:	
	...
	SD_SetMusicMode(smm_AdLib);				printf("SD_SetMusicMode(smm_AdLib)\n");
	SD_SetSoundMode(sdm_AdLib);				printf("SD_SetSoundMode(sdm_AdLib)\n");
	SD_SetDigiDevice(sds_SoundBlaster);		printf("SD_SetDigiDevice(sds_SoundBlaster)\n");
...
void InitGame()
{
	...
	MM_Startup(); 	printf("MM_Startup()\n");
	PM_Startup(); 	printf("PM_Startup()\n");
	CA_Startup(); 	printf("CA_Startup()\n");
	VW_Startup(); 	printf("VW_Startup()\n");
	IN_Startup(); 	printf("IN_Startup()\n");
	SD_Startup(); 	printf("SD_Startup()\n");
	US_Startup(); 	printf("US_Startup()\n");
	
//
// build some tables
//
	InitDigiMap();	printf("InitDigiMap()\n");
	...
	ReadConfig();	printf("ReadConfig()\n");

/* load in and lock down some basic chunks */

	CA_CacheGrChunk(STARTFONT);		printf("CA_CacheGrChunk(STARTFONT)\n");
	CA_CacheGrChunk(STARTTILE8);	printf("CA_CacheGrChunk(STARTTILE8)\n");
	for (i = LATCHPICS_LUMP_START; i <= LATCHPICS_LUMP_END; i++)
		CA_CacheGrChunk(i);
			
	BuildTables();	printf("BuildTables()\n");
	SetupWalls();	printf("SetupWalls()\n");

	NewViewSize(viewsize);	printf("NewViewSize()\n");


//
// initialize variables
//
	InitRedShifts();	printf("InitRedShifts()\n");

	IN_CheckAck();		printf("IN_CheckAck()\n");
//
// HOLDING DOWN 'M' KEY?
//
#ifndef SPEARDEMO
...
//
	...
	LastDemo = 0;				printf("DemoLoop()\n");
	
	StartCPMusic(INTROSONG);	printf("StartCPMusic(INTROSONG)\n");
