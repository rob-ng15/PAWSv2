...
{
    for (int i = 0; i < 256; i++) {
        set_palette( i, PACKRGB( palette[i * 3], palette[i * 3 + 1], palette[i * 3 + 2] ) );
    }
}
...
void VID_Init (unsigned char *palette)
{
	...
	s_framebuffer = (byte *)(0x2000000 + ( ( SCREENHEIGHT - BASEHEIGHT) >> 1 ) * SCREENWIDTH );
