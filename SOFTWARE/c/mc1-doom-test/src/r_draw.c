...
static void R_DrawColumnKernel (const byte* const src,
...
static int R_DrawFuzzColumnKernel (byte* dst, int fuzz, const int count)
{
    ...
    for (int i = count; i >= 0; --i)
    {
        ...
        // USE PIXELBLOCK TO WRITE THE PIXEL
        unsigned char volatile *GPU_REGS_B = (unsigned char volatile *)GPU_REGS; GPU_REGS_B[0x70] = colormap[dst[fuzzoffset[fuzz]]];
        dst += SCREENWIDTH;
...
void R_DrawColumn (void)
{
    ...
    R_DrawColumnKernel (dc_source, dc_colormap, frac, fracstep, count);
    GPU_REGS_B[0x78] = 0;
}
...
{
    static int          fuzzpos = FUZZTABLE-1;
    int                 count;
    byte*               dest;
